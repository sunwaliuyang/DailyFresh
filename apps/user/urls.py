...
urlpatterns = [
    ...
    re_path(r'^$', login_required(UserInfoView.as_view()), name='user'),#使用正则匹配url
    re_path(r'^order', login_required(UserOrderView.as_view()), name='order'),#使用正则匹配url
    re_path(r'^address', login_required(UserSiteView.as_view()), name='address'),#使用正则匹配url
